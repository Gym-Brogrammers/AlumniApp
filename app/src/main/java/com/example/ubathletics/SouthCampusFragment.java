...
public class SouthCampusFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        this.inflatedView = inflater.inflate(R.layout.south_map_layout, container, false);
        ...
        if(data[hour]<=20){
            inflatedView.findViewById(R.id.clarkButton).setBackgroundColor(ContextCompat.getColor(getContext(), R.color.green));
        }
        else if(data[hour]>20 && data[hour]<=40){
            inflatedView.findViewById(R.id.clarkButton).setBackgroundColor(ContextCompat.getColor(getContext(), R.color.yellow));
        }
        if(data[hour]>40){
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        inflatedView.findViewById(R.id.clarkButton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                FragmentTransaction trans = getActivity().getSupportFragmentManager().beginTransaction();
                trans.replace(getActivity().findViewById(R.id.content_frame).getId(), new ClarkGymFragment());
                trans.commit();
            }
        });
    }
